...
export const getStationsByPrefecture = async (prefecture) => {
  return db().stations.filter(
    (station) => station.prefecture.toLowerCase() == prefecture.toLowerCase(),
  );
};

export const getStationsByFacility = async (facility) => {
  return db().stations.filter((station) =>
    station.facilities.includes(facility),
  );
};
...
export const getAvailableSchedulesByRoute = async (routeId) => {
  return db().schedules.filter(
    (schedule) => schedule.routeId == routeId && schedule.status == true,
  );
};
...
export const getTicketClassByName = async (className) => {
  return (
    db().ticketClasses.find(
      (tc) => tc.class.toLowerCase() == className.toLowerCase(),
    ) || null
  );
};
...
export const getCompleteRouteDetails = async (routeId) => {
  ...
  const endStation = await getStationById(route.endStation);
  const routeSchedules = await getSchedulesByRoute(routeId);
  const operatingMonthsText = route.operatingMonths.map(getMonthAbbreviation);
